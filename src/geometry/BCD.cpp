...
bool BCD::computeBCDFromPolygonWithHoles(std::vector<Polygon_2>* polygons) {
  ...
  for (size_t times = 0; times < 4; ++times) {
    ...
    while (!edge_list_.empty()) {
      ...
      find_next_event(first_vertex, next_event, outer,
                      created_polygons_[polygon_nr_].front(),
                      created_polygons_[polygon_nr_].back());
      ...
      innerPolygonEnd(first_vertex, next_event, upper_vertices, edge_upper,
                      edge_lower, outer);
      polygon_nr_++;
    }
...
void BCD::closeSecondEvent(double x_current, double y_event, bool upper) {
  for (size_t i = 0; i < all_inner_events_.size(); ++i) {
    for (size_t a = 0; a < all_inner_events_[i].size(); ++a) {
      updateEventStatus(all_inner_events_[i][a], upper, x_current, y_event);
    }
  }
  for (size_t i = 0; i < outer_events_.size(); ++i) {
    updateEventStatus(outer_events_[i], upper, x_current, y_event);
  }
}

void BCD::updateEventStatus(Event& event, bool upper, double x_current,
                            double y_event) {
...
void BCD::closePolygon(double x_current, std::vector<Edge>& upper_vertices,
...
void BCD::createVertices(bool first_vertex, Event* event,
  ...
  if (abs(upper_point - new_point.y()) < kEps) {
    ...
    if (outer) {
      addEvent(created_polygons_[polygon_nr_], two_vertices, new_point2,
               direction2, new_point);
    } else {
      addEvent(created_polygons_[polygon_nr_], two_vertices, new_point,
               direction2, new_point2);
    }
    updateEdgeList(event, created_polygons_[polygon_nr_].back().loc,
                   direction2);
  }
}
...
void BCD::addEvent(std::vector<Edge>& poly, bool two_vertices, Point_2 point1,
