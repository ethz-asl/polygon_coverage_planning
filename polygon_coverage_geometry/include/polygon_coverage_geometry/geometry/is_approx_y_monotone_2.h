...
#ifndef MAV_2D_COVERAGE_PLANNING_GEOMETRY_IS_APPROX_Y_MONOTONE_2_H_
#define MAV_2D_COVERAGE_PLANNING_GEOMETRY_IS_APPROX_Y_MONOTONE_2_H_
...
bool
is_approx_y_monotone_2(InputIterator first, InputIterator last,
                const Traits& traits)
{
   ...
   const typename Traits::FT kPrecision = 1.0e-6;
   for (c++; c != north_c; c++)
   {
     Point_2 approx_point = Point_2(c->x(), c->y() + kPrecision);
     if (!less_yx(*prev_c, approx_point)) {
       return false;
     }
     prev_c = c;
   }
   ...
   c = north_c;
   for (c++; c != south_c; c++) {
     Point_2 approx_point = Point_2(prev_c->x(), prev_c->y() + kPrecision);
     if (!less_yx(*c, approx_point)) {
       return false;
     }
     prev_c = c;
   }
...
is_approx_y_monotone_2(InputIterator first, InputIterator last)
{
   ...
   return is_approx_y_monotone_2(first, last, K());
}

}

#endif // MAV_2D_COVERAGE_PLANNING_GEOMETRY_IS_APPROX_Y_MONOTONE_2_H_
